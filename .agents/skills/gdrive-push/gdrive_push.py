...
# No trailing \b: Office XML runs concatenate without separators, so the code is
# routinely followed immediately by a letter ("TGS-2025052468Conducted by"), which a
# word boundary would reject. (?!\d) still stops the digit run at its true end.
CODE_RE = re.compile(r"\bTGS-\d{6,}(?!\d)")
...
def _lms_api_key():
    """LMS-TMS authenticates API calls with a service key in the x-api-key header.
    Without it every request is anonymous and returns 401, which silently pushed
    this script onto the .env fallback. Same resolution order as lms_push.py."""
    import re as _re
    ...
    for path in (os.path.join(".claude", "lms_tms_api_key"),
                 os.path.join(".claude", ".env"), ".env",
        ...
        try:
            ...
            continue
        if path.endswith("lms_tms_api_key"):
            v = text.strip()
            if v:
                return v
        m = _re.search(r"^\s*(?:export\s+)?(?:LMS_TMS_API_KEY|EXTERNAL_API_KEY_FOR_CLAWDBOT)"
                       r"\s*=\s*['\"]?([^'\"\n#]+)", text, _re.M)
        if m:
...
def push_labs(root, labs_dir, dry):
    ...
    # .mcp.json carries MCP server env blocks with live API tokens (Apify, Supabase, …)
    # and settings.local.json is personal machine config; both are routinely gitignored,
    # so git never warns about them — but rclone does not read .gitignore.
    # .playwright-mcp/ holds console/page debug dumps that can contain submitted form data.
    for pat in (".env", "*.pem", "*.key",
                ".mcp.json", "settings.local.json", ".playwright-mcp/**",
...
def main():
    ...
    # Courses name these either "LG-*.docx" or "Learner Guide - *.docx" (same for the LP),
    # so try the short prefix first and fall back to the spelled-out name.
    def newest_either(*patterns):
        for pat in patterns:
            hit = newest(os.path.join(cw, pat))
            if hit:
                return hit
        return None

    lg_docx = newest_either("LG-*.docx", "Learner Guide*.docx")
    lg_pdf = newest_either("LG-*.pdf", "Learner Guide*.pdf")
    lp_docx = newest_either("LP-*.docx", "Lesson Plan*.docx")
    lp_pdf = newest_either("LP-*.pdf", "Lesson Plan*.pdf")
    # Facilitator Guide and Assessment Plan ride along in the Lesson Plan folder.
    fg_docx = newest_either("FG-*.docx", "Facilitator Guide*.docx")
    fg_pdf = newest_either("FG-*.pdf", "Facilitator Guide*.pdf")
    ap_docx = newest_either("AP-*.docx", "Assessment Plan*.docx")
    ap_pdf = newest_either("AP-*.pdf", "Assessment Plan*.pdf")
    ...
    routing = [
        ...
        ("Lesson Plan", "lesson plan",
         [lp_docx, lp_pdf, fg_docx, fg_pdf, ap_docx, ap_pdf]),
